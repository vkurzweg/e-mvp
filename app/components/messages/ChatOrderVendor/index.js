...
function ChatOrderVendor(props) {
  ...
  return (
    <div style={{ textAlign: 'center', marginBottom: '10%', borderBottom: '1px solid #BDBEBD' }}>
      <Avatar src={vendorPhoto} size={80} />
      <h4 style={{ textTransform: 'uppercase' }}>{vendorName}</h4>
      <p style={{ marginBottom: '10%' }}>{vendorAddress} | {vendorCity} | {vendorZip} </p>
    </div>
  );
}
