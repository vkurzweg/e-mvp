...
import VendorOrderItemMobile from '../../../components/VendorOrderItemMobile';
...
export class VendorOrderContainer extends React.Component { // eslint-disable-line react/prefer-stateless-function
  ...
  handleConfirmOrder() {
    console.log('confirm order clicked', this.props.order._id);
  ...
  showModal() {
    this.props.setOrderId(this.props.order._id);
  ...
  render() {
    ...
    (this.props.order.status === 'ACCEPTED' || this.props.order.status === 'DECLINED') ? displayButton = 'none' : displayButton = 'block';
    // date format
    ...
    let orderTotal = prices.reduce((a, b) => a + b, 0);
      return (
        <div style={{ display: 'block', margin: '0 auto', marginTop: '5%' }}>
          <div style={{ display: 'inline-block', textAlign: 'center', marginLeft: '77%' }}>
          ...
          <div style={{ display: 'block', margin: '0 auto'}}>
            <p style={{ textTransform: 'uppercase', textAlign: 'center', marginTop: '-10%' }} ><b>{customer}</b></p>
            <table style={{ marginLeft: '32%', marginBottom: '5%', fontSize: '12px' }}>
              <tbody>
                <tr>
          ...
          <div style={{ marginBottom: '5%', float: 'right', marginRight: '15%', marginBottom: '10%' }}>
            <p style={{ fontSize: '14px', display: 'block', textAlign: 'center', marginBottom: '15%'}}>Order Total: ${orderTotal}</p>
          </div>
          <button
            className="btn btn-success"
            style={{ display: displayButton, width: '10em', backgroundColor: '#2FBA90', textAlign: 'center', border: 'none', margin: '0 auto', marginBottom: '2%', marginTop: '10%', marginLeft: '33%', boxShadow: '10px 10px 40px -17px rgba(37,39,41,1)', outline: 'none' }}
            onClick={this.showModal.bind(this, this.props.order._id)}
          >
              Accept Order
          </button>
          <button className="btn btn-warning"
            style={{ display: displayButton, backgroundColor: '#FF7A7A', width: '10em', border: 'none', textAlign: 'center', margin: '0 auto', marginBottom: '1em', marginTop: '3%', marginLeft: '33%' }}
